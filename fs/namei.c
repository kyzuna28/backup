...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (nd->inode &&
		unlikely(test_bit(AS_FLAGS_SUS_PATH, &nd->inode->i_mapping->flags) && likely(susfs_is_current_proc_umounted()))) {
		return -ENOENT;
	}
#endif
...
static int may_linkat(struct path *link)
{
	...
	if (link->dentry->d_inode &&
		unlikely(test_bit(AS_FLAGS_SUS_PATH, &link->dentry->d_inode->i_mapping->flags) && likely(susfs_is_current_proc_umounted()))) {
		return -ENOENT;
	}
#endif
			   ...
			   struct path *path, struct path *root)
{
	...
	if (!retval && path->dentry->d_inode &&
		unlikely(test_bit(AS_FLAGS_SUS_PATH, &path->dentry->d_inode->i_mapping->flags) && likely(susfs_is_current_proc_umounted()))) {
		putname(name);
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode &&
		unlikely(test_bit(AS_FLAGS_SUS_PATH, &child->d_inode->i_mapping->flags) && likely(susfs_is_current_proc_umounted()))) {
		error = inode_permission2(mnt, dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
...
static int may_o_create(const struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (dentry->d_inode &&
		unlikely(test_bit(AS_FLAGS_SUS_PATH, &dentry->d_inode->i_mapping->flags) && likely(susfs_is_current_proc_umounted()))) {
